...
describe('StringUtils.toDashCase', () => {
	...
	it('returns dash case string (conservative)', () => {
		...
		expect(toDashCase(string, true)).to.be.equal('this-i-s-the-c-a-s-e');
	});

	it('returns dash case string (conservative with numbers)', () => {
		const string = 'ThisISThe01CASE';

		expect(toDashCase(string, true)).to.be.equal('this-i-s-the-01-c-a-s-e');
	});
